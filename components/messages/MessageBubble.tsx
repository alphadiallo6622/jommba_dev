...
export default function MessageBubble({ message }: Props) {
  ...
  // Un vocal en cours d'envoi n'a pas encore de chemin : la bulle s'affiche
  // quand même, avec son état « envoi en cours ».
  const isVoice = Boolean(message.audioPath) || message.audioPending === true

  return (
    <div className={`flex ${isMine ? 'justify-end' : 'justify-start'} mb-2.5 animate-fade-in-up`}>
      <div className={`max-w-[80%] sm:max-w-[min(70%,620px)] px-3.5 py-2.5 text-[14px] leading-relaxed ${
        isVoice ? '' : 'whitespace-pre-wrap break-words'
      } ${
        isMine
        ...
        {isVoice ? (
          <VoiceMessagePlayer
            ...
            isMine={isMine}
            pending={message.audioPending === true}
          />
        ) : (
          message.text
        )}
