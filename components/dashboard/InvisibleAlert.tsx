...
export default function InvisibleAlert() {
  ...
  const t = useTranslations('dashboard.invisibleAlert')
  const { profileCompletion } = useCurrentUser()
  const mode = useVisibilityStore(s => s.mode)
  const bold = { b: (chunks: React.ReactNode) => <span className="font-semibold">{chunks}</span> }
    ...
    return (
      <div className="flex flex-col gap-2">
        <div className="rounded-xl p-4 bg-red-50 border border-red-200 flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-red-700 text-sm">{t('pauseTitle')}</p>
            <p className="text-xs text-red-500 mt-0.5">{t('pauseDesc')}</p>
          </div>
        </div>
        <div className="rounded-xl px-4 py-2.5 bg-amber-50 border border-amber-200">
          <p className="text-xs text-amber-700">{t.rich('pauseModeNote', bold)}</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="flex flex-col gap-2">
        <div className="rounded-xl p-4 bg-pink-50 border border-pink-200 flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-pink-700 text-sm">{t('discussionTitle')}</p>
            <p className="text-xs text-pink-500 mt-0.5">{t('discussionDesc')}</p>
          </div>
        </div>
        <div className="rounded-xl px-4 py-2.5 bg-pink-50 border border-pink-200">
          <p className="text-xs text-pink-600">{t.rich('discussionModeNote', bold)}</p>
        </div>
      </div>
    )
  }
    ...
    <div className="rounded-xl p-4 border border-amber-200" style={{ background: '#FEF3C7' }}>
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-gray-900 text-sm">{t('incompleteTitle')}</span>
        </div>
