...
import { z } from 'zod'
import {
  Mail, Lock, Eye, EyeOff, Shield, Users, CheckCircle2,
...
/* ─── Password strength ─── */
function getStrength(pwd: string, labels: { weak: string; medium: string; strong: string }): { level: number; label: string; color: string } {
  ...
  if (score <= 1) return { level: 1, label: labels.weak,   color: 'bg-red-500' }
  if (score <= 3) return { level: 2, label: labels.medium, color: 'bg-amber-500' }
  return              { level: 3, label: labels.strong,  color: 'bg-emerald-500' }
}
...
export default function InscriptionPage() {
  ...
  const step1Schema = z.object({
    ...
    lastName:  z.string().min(2, t('errorLastNameMin')),
  ...
  const onStep1 = async (data: Step1Data) => {
    ...
    try {
      ...
      if (!result.valid) {
        setError1('email', { type: 'manual', message: result.reason ?? t('errorEmailUnreachable') })
        return
      }
    } catch {
        ...
        <div className="w-full max-w-sm space-y-6">
            ...
            <>
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                ...
                <span className="text-xs text-gray-600 leading-relaxed">
                  {t.rich('consent', {
                    terms: (chunks) => (
                      <a href="#" className="font-semibold underline" style={{ color: '#10B981' }}>{chunks}</a>
                    ),
                    charter: (chunks) => (
                      <a href="#" className="font-semibold underline" style={{ color: '#10B981' }}>{chunks}</a>
                    ),
              ...
              <form onSubmit={hs1(onStep1)} className="space-y-4">
                {/* Prénom */}
                <div>
                  <label className="text-xs font-semibold text-gray-700 block mb-1">{t('firstNameLabel')}</label>
                  <div className="relative">
                ...
                {/* Nom */}
                <div>
                  <label className="text-xs font-semibold text-gray-700 block mb-1">{t('lastNameLabel')}</label>
                  <div className="relative">
                    <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                    <input
                      type="text"
          ...
          {step === 2 && (
            <form onSubmit={hs2(onStep2)} className="space-y-4">
              {/* Password */}
              <div>
                <label className="text-xs font-semibold text-gray-700 block mb-1">{t('passwordLabel')}</label>
                <div className="relative">
          ...
          <div className="border-t border-gray-50 pt-4 text-center">
            <p className="text-xs italic text-gray-400 leading-relaxed">
              {tShared('quoteGarment')}
            </p>
            <p className="text-xs text-emerald-500 font-medium mt-0.5">{tShared('quoteGarmentSource')}</p>
          </div>
        </div>
      </div>
    </div>
