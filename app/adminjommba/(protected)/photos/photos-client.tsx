"use client";
// app/adminjommba/(protected)/photos/photos-client.tsx
import { useState, useEffect, useCallback, useTransition } from "react";
import { useRouter } from "next/navigation";
import { X, ChevronLeft, ChevronRight, ImageOff } from "lucide-react";
import type { PhotoQueueItem } from "@/lib/admin/types";
import { approvePhoto, rejectPhoto } from "@/app/adminjommba/actions";
...
function PhotoLightbox({
  ...
  initialPhoto: PhotoQueueItem;
  memberPhotos: PhotoQueueItem[];
  busy: boolean;
  onClose: () => void;
  onApprove: (p: PhotoQueueItem) => void;
  onReject: (p: PhotoQueueItem) => void;
}) {
  const [idx, setIdx] = useState(() =>
    Math.max(0, memberPhotos.findIndex((p) => p.id === initialPhoto.id)),
  );
  const total   = memberPhotos.length;
  const current = memberPhotos[Math.min(idx, total - 1)];
    ...
    >
      ...
      >
        <button
          ...
          className="px-7 py-2.5 rounded-xl border border-red-400/60 text-red-300 text-sm font-semibold hover:bg-red-900/40 transition-colors disabled:opacity-50"
        >
          Rejeter
        </button>
        <button
          ...
          className="px-7 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 transition-colors disabled:opacity-50"
        >
...
/* ── Page ─────────────────────────────────────────────────────────────────── */
export function PhotosClient({ photos }: { photos: PhotoQueueItem[] }) {
  const { show } = useToast();
  const router = useRouter();
  const [busy, startTransition] = useTransition();
  const [removed, setRemoved] = useState<Set<string>>(new Set());
  const [viewing, setViewing] = useState<PhotoQueueItem | null>(null);

  const queue = photos.filter((p) => !removed.has(p.id));

  const photosByMember = (memberId: string) =>
    queue.filter((p) => p.memberId === memberId);

  const afterModeration = (photo: PhotoQueueItem) => {
    setRemoved((prev) => new Set(prev).add(photo.id));
    const remaining = photosByMember(photo.memberId).filter((p) => p.id !== photo.id);
    setViewing(remaining.length > 0 ? remaining[0] : null);
    router.refresh();
  };

  const handleApprove = (photo: PhotoQueueItem) => {
    startTransition(async () => {
      const res = await approvePhoto(photo.id);
      if (res.ok) {
        show(`Photo approuvée · ${photo.name}`, "success");
        afterModeration(photo);
      } else {
        show(res.error ?? "Erreur lors de l'approbation", "error");
      }
    });
  };

  const handleReject = (photo: PhotoQueueItem) => {
    startTransition(async () => {
      ...
      if (res.ok) {
        ...
        show(res.error ?? "Erreur lors du rejet", "error");
      }
    });
  };
