"use client";
// app/adminjommba/(protected)/academie/academie-client.tsx
// Gestion éditoriale des articles de l'Académie du Mariage — même modèle que le blog.
import { useRef, useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { PlusCircle, X, ChevronLeft, ChevronRight, GraduationCap, ImagePlus, Loader2 } from "lucide-react";
import type { AcademyArticleRow, PostStatus } from "@/lib/admin/types";
import { saveAcademyArticle, setAcademyArticleStatus } from "@/app/adminjommba/actions";
import { RichTextEditor } from "@/components/admin/ui/rich-text-editor";
import { useToast } from "@/components/admin/ui/toast";

const PER_PAGE = 9;

const CATEGORIES = ["Conseils pratiques", "Hadiths & Sagesse", "Histoires inspirantes", "Droits & Devoirs", "Préparation", "Spiritualité"];

const CATEGORY_GRADIENT: Record<string, string> = {
  "Conseils pratiques":    "from-amber-500 to-orange-400",
  "Hadiths & Sagesse":     "from-emerald-500 to-teal-400",
  "Histoires inspirantes": "from-teal-500 to-cyan-400",
  "Droits & Devoirs":      "from-yellow-500 to-amber-400",
  "Préparation":           "from-blue-500 to-cyan-400",
...
const EMPTY_FORM: EditForm = {
  id: "", title: "", category: "Conseils pratiques", author: "Équipe Jommba",
...
/* ── Page ─────────────────────────────────────────────────────────────────── */
export function AcademieClient({ articles }: { articles: AcademyArticleRow[] }) {
  const { show } = useToast();
  ...
  const openEdit = (post: AcademyArticleRow) =>
    setForm({
      id:         post.id,
      title:      post.title,
  ...
  const handleSave = (status: PostStatus) => {
    ...
    startTransition(async () => {
      const res = await saveAcademyArticle({
  ...
  const handleStatus = (post: AcademyArticleRow, status: PostStatus) => {
    startTransition(async () => {
      const res = await setAcademyArticleStatus(post.id, status);
      if (res.ok) {
        show(
          status === "published"
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold text-[var(--color-ink)]">Académie du Mariage</h1>
            <p className="text-sm text-[var(--color-muted)] mt-0.5">
              Articles pédagogiques visibles par les membres dans l&rsquo;académie.
            </p>
          </div>
          <button
            onClick={openNew}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between pt-1">
            <span className="text-xs text-[var(--color-muted)]">
              {articles.length} articles · page {page} / {totalPages}
            </span>
