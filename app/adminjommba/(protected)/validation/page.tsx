"use client";
// app/admin/(protected)/validation/page.tsx
import { useState } from "react";
import { X, AlertCircle, ImageOff } from "lucide-react";
import { PENDING_PROFILES, type PendingProfile } from "@/lib/admin/mock-data";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="h-24 bg-gradient-to-br from-[var(--color-brand-700)] to-[var(--color-brand-400)] rounded-t-2xl relative shrink-0">
          ...
          {/* Avatar overlapping */}
          <div className="absolute bottom-0 left-6 translate-y-1/2 w-14 h-14 rounded-full bg-white flex items-center justify-center shadow-lg border-4 border-white">
        ...
        <div className="pt-10 px-6 pb-6 space-y-5">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--color-muted)] mt-0.5">
              {profile.gender} · {profile.city}, {profile.country} · soumis {profile.sub}
            </p>
          </div>
          ...
          <div>
            <p className="text-sm font-semibold text-[var(--color-ink)] mb-3">
              Photos soumises ({profile.photos})
            </p>
            <div className="grid grid-cols-3 gap-2.5">
              {Array.from({ length: profile.photos }).map((_, i) => (
                <div
                  key={i}
                  className="aspect-square bg-[var(--color-faint)] rounded-xl flex items-center justify-center border border-[var(--color-line)]"
                >
                  <ImageOff className="w-6 h-6 text-[var(--color-line-2)]" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            <button
              onClick={() => { onClose(); show(`Profil refusé · ${reason}`, "error"); }}
              className="py-3 rounded-xl border border-red-200 text-red-600 text-sm font-semibold hover:bg-red-50 transition-colors"
            >
              Refuser
            </button>
            <button
              onClick={() => { onClose(); show("Profil validé", "success"); }}
              className="py-3 rounded-xl bg-[var(--color-brand-600)] text-white text-sm font-semibold hover:bg-[var(--color-brand-700)] transition-colors"
            >
              Valider le profil
            </button>
          </div>
...
export default function ValidationPage() {
  const { show }    = useToast();
  const [examining, setExamining] = useState<PendingProfile | null>(null);
  ...
  return (
    <>
      <div className="space-y-4">
        <div className="flex items-start justify-between">
          ...
          <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200 shrink-0">
            {PENDING_PROFILES.length} en attente
          </span>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {PENDING_PROFILES.map((p, i) => (
            <div
              key={i}
              className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-line)] shadow-[var(--shadow-card)] overflow-hidden"
            >
              {/* Photo header */}
              <div className="relative h-28 bg-gradient-to-br from-[var(--color-brand-600)] to-[var(--color-brand-400)]">
                ...
                <div className="absolute bottom-0 left-4 translate-y-1/2 w-11 h-11 rounded-full bg-white flex items-center justify-center shadow-sm border-2 border-white">
              ...
              <div className="pt-8 px-4 pb-4 space-y-2.5">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => show(`Profil validé · ${p.name}`, "success")}
                    className="flex-1 py-2 rounded-lg bg-[var(--color-brand-600)] text-white text-xs font-semibold hover:bg-[var(--color-brand-700)] transition-colors"
                  >
                    Valider
                  </button>
                </div>
      ...
      {/* Modal */}
      {examining && (
        <ExamineModal profile={examining} onClose={() => setExamining(null)} />
      )}
    </>
  );
}
